# -*- coding: utf-8 -*-
"""
v160 - TARJETAS CONTANDO ROJAS: la magnitud que liquida la casa.
...
def carga(clave):
    ...
    for c in ('home_yellow', 'away_yellow', 'home_red', 'away_red'):
        d[c] = pd.to_numeric(d[c], errors='coerce')
    d = d.dropna(subset=['home_yellow', 'away_yellow',
                         'home_red', 'away_red']).reset_index(drop=True)
    # LA MAGNITUD QUE LIQUIDA LA CASA. Medido contra 14 partidos con linea
    # real, el modelo de solo amarillas queda 0,27 tarjetas por debajo del
    # centro de la casa, y las rojas valen 0,25 de media. Asi que su mercado
    # «Total de tarjetas» cuenta las dos cosas, y modelar solo amarillas
    # sesgaria a la baja todo EV que se calcule despues contra esa linea.
    d['home_yellow'] = d['home_yellow'] + d['home_red']
    d['away_yellow'] = d['away_yellow'] + d['away_red']
    return d
...
json.dump({'dispersion': resumen_disp, 'estimadores': filas, 'arbitro': filas_ref},
          open('_v160_tarjetas_con_rojas.json', 'w', encoding='utf-8'),
